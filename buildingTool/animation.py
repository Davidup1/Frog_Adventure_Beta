...
class Animation:
    ...
    def landing(self,duration, height=300, offset=300):
        self.set_circulation(0, 0)
        self.type = "landing"
        self.duration = duration
        self.animationList = [0] * duration
        for i in range(duration):
            self.animationList[i] = round(height * cos(pi * i / duration/2+pi/2))+offset
        self.reset()

    def linear(self,duration, height=30):
        self.set_circulation(0, 0)
        self.type = "liner"
        self.duration = duration
        self.animationList = [0] * duration
        for i in range(duration):
            self.animationList[i] = round(height * i / duration)
        self.reset()
    ...
    def play(self, data, mode=0):
        ...
        if not self.finish:
            ...
            elif self.type in ["float", "quadratic", "peak", "shake", "landing"]:  # data为rect
